...
| STUDIO-CORE-012 | Framework-specific coding assistance: completion, references, injections, line markers, navigation, inspections, intentions, quick fixes and safe refactorings | PARTIAL | Native FlowUI XML IDs, Java/Kotlin controller annotations, entity classes, nested entity properties, shared/inline fetch plans, message keys, view/screen IDs, nested menu IDs, resource/row policy targets and specific permissions now provide completion, navigation, Find Usages, project-wide cross-file rename, bidirectional controller/descriptor gutters, ambiguity diagnostics and quick fixes. Seven independent persistent indexes replace global PSI invalidation and broad file enumeration for entity, view, menu, message, permission, fetch-plan and FlowUI descriptor discovery; a dual-host 3,000-file fixture proves cross-artifact cache isolation and millisecond warm/incremental access. Complete service/event/configuration/menu-bean/UI-component coverage, language injections, intentions and installed-IDE cold-index/p50-p95-p99/memory/leak proof remain |
