...
| STUDIO-CORE-011 | Live database reverse engineering, partial reverse engineering, entity/schema mapping, type mapping and safe regeneration | PARTIAL | Existing entities now inspect a selected live table through the project-owned active-profile JDBC driver, including datasource-only stores with Liquibase disabled. Credential-free digest snapshots cover columns, PKs, imported FKs, indexes and dependencies; official scalar families and known mapped relationships are proposed in a responsive review surface and selected additions enter the existing revision-bound Java/Kotlin atomic preview, while generated/PK/already-mapped/vendor-specific unsafe columns fail closed. Dual-host metadata integration and secret-boundary tests pass. Complete catalog/table/view selection, composite keys, join tables, dependency import, saved mapping overrides, repeatable regeneration and populated PostgreSQL/MySQL/MariaDB/MSSQL/Oracle loss-prevention fixtures remain; see `ENTITY-DESIGNER-PARITY.md` |
