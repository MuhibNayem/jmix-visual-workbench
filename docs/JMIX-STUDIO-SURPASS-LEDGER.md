...
| STUDIO-CORE-012 | Framework-specific coding assistance: completion, references, injections, line markers, navigation, inspections, intentions, quick fixes and safe refactorings | PARTIAL | Native FlowUI XML IDs, Java/Kotlin controller annotations, entity classes, nested entity properties, shared/inline fetch plans, message keys, view/screen IDs, nested menu IDs, Spring menu bean names/methods, resource/row policy targets and specific permissions now provide completion, navigation, Find Usages, project-wide cross-file rename, bidirectional controller/descriptor gutters, ambiguity diagnostics and quick fixes. Eight independent persistent indexes replace global PSI invalidation and broad file enumeration for entity, view, Spring bean, menu, message, permission, fetch-plan and FlowUI descriptor discovery; a dual-host 3,000-file fixture plus repeated typing cycles prove cross-artifact cache isolation and millisecond warm/incremental access. Complete service/event/configuration/UI-component coverage, custom composed-stereotype and Kotlin `@Bean` factory support, language injections, intentions and installed-IDE cold-index/p50-p95-p99/memory/leak proof remain |
...
| CERT-PERF-001 | Published indexing, UI latency, generation, memory and leak budgets | PARTIAL | Dual-host 3,000-file regression proves 25 warm inventory reads in 1 ms and unrelated-edit access in 1–2 ms without cross-artifact eviction. Add installed-IDE cold indexing, completion/navigation p50/p95/p99, generation, memory, cancellation and leak baselines |
