...
| CERT-KOTLIN-001 | Kotlin discovery and editing wherever Kotlin support is declared | PARTIAL | Entity generation, source-set targeting, repository generation, Kotlin schema-metadata reconstruction, revision-bound additive handwritten-entity editing, managed mapping evolution, native stable-column property rename, project-wide scalar Type Migration through Kotlin light fields, native Safe Delete preparation and atomic explicit scalar plus owning-to-one physical-column rename now pass on both host lanes. Relationship-shape and data-preserving physical conversion/retirement choreography plus target-project semantic compilation remain |
