...
| STUDIO-CORE-011 | Live database reverse engineering, partial reverse engineering, entity/schema mapping, type mapping and safe regeneration | PARTIAL | The active-profile, project-driver JDBC backend now browses bounded catalogs, schemas, tables and views without moving credentials or connection URLs into JCEF, including datasource-only stores with Liquibase disabled. Handwritten Java/Kotlin `@Table` metadata retains `name`, `schema`, and `catalog`. Any selected table can be inspected through a credential-free digest snapshot covering columns, PKs, imported FKs, indexes and dependencies, but merge controls unlock only when the backend proves the exact expected entity/store/table/schema/catalog mapping; same-named cross-schema objects stay read-only. Official scalar families and known mapped relationships enter the existing revision-bound Java/Kotlin atomic preview, while generated/PK/already-mapped/vendor-specific unsafe columns fail closed. The fluid browser layout and both lock/unlock paths were rendered in a real browser without runtime errors; dual-host metadata, qualifier, and secret-boundary tests pass. Composite keys, join tables, recursive dependency import, saved mapping overrides, repeatable regeneration and populated PostgreSQL/MySQL/MariaDB/MSSQL/Oracle loss-prevention fixtures remain; see `ENTITY-DESIGNER-PARITY.md` |
...
| CERT-JMIX-001 | Explicit Jmix 2.x/3.x version adapters; only tested cells are advertised | PARTIAL | The published matrix now pins and compiles exact 2.8.2 and 3.0.0 framework graphs with line-aware aggregate delegate generation. Complete operation-by-operation adapters, real application startup and representative customer-project fixtures remain |
