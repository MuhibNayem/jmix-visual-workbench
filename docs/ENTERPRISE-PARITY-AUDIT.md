...
- The workflow palette, subprocess inspector, Jmix email inspector and scenario
  ...
  errors.
- IntelliJ 2025.3: 180 tests and 3 host smoke tests passed; the packaged
  plugin verifier reports compatibility with IU-253.28294.334.
- IntelliJ 2026.2: 180 tests and 3 host smoke tests passed; the packaged
...
- Thirteen native domain-assistance scenarios pass on both IntelliJ hosts,
  ...
  diagnostics.
- Ten native UI/security-assistance scenarios pass on both IntelliJ hosts,
  ...
  inspections and quick fixes. The locale regression proves that a key present
...
- Seven native FlowUI controller-contract scenarios pass on both IntelliJ
  hosts, covering valid and invalid Java/Kotlin `@ViewComponent`, `@Subscribe`,
  `@Install` and `@Supply` declarations, raw-generic repair and target-kind
  reference scoping.
- A 3,000-unrelated-file native index fixture passes on both IntelliJ hosts.
  ...
  measured 12/15 ms total on IDEA 2025.3/2026.2; access after unrelated edits
  measured 3/3 ms; twenty consecutive three-file typing cycles measured
  136/217 ms total while preserving every cache identity. A relevant message
  edit replaced only the message inventory in 55/77 ms. The architectural
...
1. Complete native Jmix editor intelligence across service methods, events,
   configuration and UI component policies, add custom composed-stereotype and
