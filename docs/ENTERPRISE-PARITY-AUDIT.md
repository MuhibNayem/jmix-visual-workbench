...
| Integration designer | Substantial | Responsive permanent catalog/canvas/inspector workspace; dependency-aware HTTP/webhook, Kafka, RabbitMQ, SFTP, Jmix email, Jmix file/object storage, SMS, payment and identity-provider adapter models; indexed `OAuth2AuthorizedClientManager` and authorized-client-service selection with externalized registration/principal properties; named Spring Boot SSL-bundle and version-aware HTTP-runtime selection; externalized endpoints/secrets; indexed inbound-handler binding; bounded timeout, retry/backoff/DLT, delivery, transaction, ordering, idempotency, circuit-breaker, bulkhead, rate-limit and observability policies; fail-closed conflict validation; exact owned-source rediscovery/lock; first-class connector/method/service-call graph edges. Kafka/Rabbit publishers atomically add a selected-store durable outbox, Java adapter, external policy, portable Liquibase migration and root include. Kafka/Rabbit consumers can similarly add a persistent inbox that transacts handler database effects with message state, suppresses identical redelivery, rejects same-ID/different-payload collisions, quarantines missing identity, bounds payload and maintenance work, routes poison messages to DLT and gates replay/maintenance with separate Jmix permissions. Backend-owned store, Jackson, OAuth service and target-runtime resolution prevents browser-forged wiring. Generated Jmix 2.8/Jackson 2 and Jmix 3.0/Jackson 3 source compiles in all four certified Jmix/JDK cells, with explicit Spring Boot 4 Kafka and HTTP-client capability gates. Exact production output runs in disposable Jmix 2.8/Java 17 and Jmix 3.0/Java 21 applications against PostgreSQL, Kafka, RabbitMQ, SFTP, WireMock and Toxiproxy. It proves publisher migrations/proxying/outage recovery/ordering/replay/checksum/lease safety, six inbound scenarios per broker with transactional effects, SFTP atomic transfer/traversal rejection, HTTP secrets/idempotency/timeouts/retry/circuit breaking, OAuth2 client-credentials renewal and invalid-token eviction, mandatory mTLS with negative trust/identity/hostname paths, atomic hot certificate rotation with last-working-client retention, and Micrometer counters | Authorization-code login and RFC 8705 certificate-bound-token flows, organization catalog/versioning, remaining provider runtime fixtures, cross-database proof, multi-node/load/soak and installed-IDE runtime certification |
...
- The workflow palette, subprocess inspector, Jmix email inspector and scenario
  ...
  errors.
- IntelliJ 2025.3: 384 tests and 3 host smoke tests passed; the packaged
  plugin verifier reports compatibility with IU-253.28294.334.
- IntelliJ 2026.2: 384 tests and 3 host smoke tests passed; the packaged
...
4. Finish the integration runtime tranche: provider-native tracing/audit,
   authorization-code and RFC 8705 certificate-bound-token flows, signed
   organization catalogs, remaining provider runtime fixtures, cross-database
   inbox/outbox proof, multi-node/load/soak and installed-IDE runtime proof.
