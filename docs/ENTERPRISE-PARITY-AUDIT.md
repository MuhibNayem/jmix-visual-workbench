...
| Entity and schema studio | Substantial | Entity-kind-aware authoring for JPA entities, mapped superclasses, embeddables, Jmix DTOs and typed `EnumClass<String|Integer>` enumerations; official scalar datatype families and explicit custom Java/SQL datatypes; configured embedded IDs; traits, inheritance, validation groups, listeners, comments/system/LOB/property metadata, read-only accessors, relationships/compositions/cross-store references, repositories, constraints/indexes, aligned Liquibase, additive source-safe existing-Java editing, datastore-aware migration proposals, include-chain protection, schema diagnostics and connected-consumer impact | Exact handwritten Java/Kotlin PSI refactor round trip; selected attribute propagation into views/fetch plans/localization/security; composite-ID migration proof; live partial database reverse engineering; destructive migration choreography and populated multi-database rehearsal |
