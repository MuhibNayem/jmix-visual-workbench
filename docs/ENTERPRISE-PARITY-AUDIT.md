...
certification; application-startup and database proof is recorded separately
below, while full runtime coverage for every generated change class remains
open.

Database/runtime evidence update (2026-07-31): a clean disposable Docker matrix
now starts real Jmix 2.8.2 and 3.0.0 applications on PostgreSQL 16.9, MySQL
8.4.6, MariaDB 11.4.8, SQL Server 2022 CU20 and Oracle 23.7 under Java 17, 21
and 25. Sixty machine-readable results prove Jmix enhancement, metadata,
`DataManager` relationship/`BigDecimal` round trip, Liquibase baseline,
forward/rollback/reapply and released locks across twenty cells. Ten more
results execute the production reverse-engineering service on both IntelliJ
hosts and prove project-property/library-driver resolution, schema browsing,
PK/FK/unique/index reconstruction, dependency closure, existing-entity reuse,
import planning and credential redaction. The final evidence audit found zero
failed assertions or exposed URLs/passwords. See
`DATABASE-RUNTIME-CERTIFICATION.md`.
...
| Project, profile and environment configuration | Substantial | Bounded multi-root project/profile inventory; source-preserving profile edits, comparison, create/remove and exact undo; explicitly imported project-local `.env`/`.env.properties` connection and mutation; native password entry; value, source and secret-derived-hash containment across JCEF; guarded reference-aware removal; profile placeholder/group/include/cycle diagnostics; and multi-root IntelliJ launch evidence that is never mislabelled as runtime proof. The continuous three-region environment workspace passes 1280/640/440/320 responsive inspection, 23 focused tests per host and the current clean dual-host artifact gate | Previewed Gradle/settings/version/dependency mutations; data-store lifecycle and connection tests; attach-to-process runtime reconciliation; installed native-dialog accessibility and recovery certification |
...
| Jmix REST/API studio | Substantial | `rest-services.xml` and `rest-queries.xml` discovery/editing, method/query parameters, fetch plans, authorization/transaction warnings, saved invocation payloads and redacted token input. Project-owned OpenAPI 3.0/3.1 consumer contracts provide exact operation/representation binding, typed client generation, backend-enforced security requirements, a Jmix DTO/entity mapper and application-service facade, or exact revision-bound mapping to existing entities with explicit enum and converter extensions | OpenAPI provider authoring, GraphQL/gRPC/SOAP, consumer/provider contract suites and installed-IDE certification |
| OpenAPI contract clients | Substantial | Native or indexed selection of project-owned OpenAPI 3.0/3.1 JSON/YAML; bounded off-UI-thread parsing with network resolution disabled; controlled project-local multi-file JSON Pointer bundles with exact root and referenced-document SHA-256 identities; exact per-operation representation binding; typed parameters, records and enums; exact status/media handling; backend-owned security-alternative, OAuth-scope and mTLS evaluation; safe owned regeneration; four-cell generated-code compilation; and semantic contract-evolution review. One revision-bound client supports a primary plus 63 additional operations over one canonical shared-schema registry, per-operation Java signatures and representations, compact primary-owned baseline persistence, operation promotion and bundle-wide stale detection, evolution analysis, Jmix remapping, native approval and atomic regeneration. The Jmix layer supports generated DTO entities or exact existing-entity targets across the operation union. Existing `EnumClass` adapters and project Spring converter beans are discovered from bounded semantic/index evidence, filtered by destination-module visibility, bound to exact source revisions/constants/method signatures, type-checked in both directions and revalidated during preview/apply. See `OPENAPI-CONTRACT-CLIENTS.md` | Kotlin generation, consumer/provider contract suites and installed-IDE certification |
| Integration designer | Substantial | Responsive permanent catalog/canvas/inspector workspace; dependency-aware HTTP/webhook, Kafka, RabbitMQ, SFTP, Jmix email, Jmix file/object storage, SMS, payment and identity-provider adapter models; indexed `OAuth2AuthorizedClientManager` and authorized-client-service selection with externalized registration/principal properties; named Spring Boot SSL-bundle and version-aware HTTP-runtime selection; externalized endpoints/secrets; indexed inbound-handler binding; bounded timeout, retry/backoff/DLT, delivery, transaction, ordering, idempotency, circuit-breaker, bulkhead, rate-limit and observability policies; fail-closed conflict validation; exact owned-source rediscovery/lock; first-class connector/method/service-call graph edges. Kafka/Rabbit publishers and consumers atomically add portable durable outbox/inbox migrations and code with transactional effects, duplicate/collision handling, ordering, DLT and permission-gated replay/maintenance. Backend-owned store, Jackson, OAuth service and target-runtime resolution prevents browser-forged wiring. Generated Jmix 2.8/Jackson 2 and Jmix 3.0/Jackson 3 source compiles in all four certified Jmix/JDK cells. Exact production output runs in disposable Jmix 2.8/Java 17 and Jmix 3.0/Java 21 applications against PostgreSQL, Kafka, RabbitMQ, SFTP, WireMock and Toxiproxy, proving outage recovery, ordering, replay, inbox/outbox failure safety, SFTP traversal rejection, HTTP resilience, OAuth2 renewal/eviction, mandatory mTLS negative paths, hot certificate rotation and Micrometer counters. OpenAPI connectors add revision-bound shared multi-operation clients, Jmix DTO/existing-entity mapping across the operation union, controlled transitive project-local contract bundles, explicit `EnumClass` value translation and exact bidirectional project-converter invocation, with stale/forged/inaccessible symbols rejected by the backend. Schema-v3 Ed25519/SHA-256 organization catalogs provide exact-version compatible connector presets with backend-enforced authentication, transport, reliability and telemetry policy. Catalogs cannot contain executable code, endpoints or secret values; sensitive/restricted presets require a five-minute memory-only native IntelliJ approval bound to exact digest/template/module/policy. Native authoring, signing, review, import/refresh/offline administration, multi-team key-rotation/anti-rollback tests, 1280/720/360 responsive evidence and the clean dual-host gate pass; see `OPENAPI-CONTRACT-CLIENTS.md` and `ORGANIZATION-CONNECTOR-CATALOGS.md` | GraphQL/SOAP/event schema mapping; authorization-code login and RFC 8705 certificate-bound-token flows; remaining provider runtime fixtures; cross-database proof; multi-node/load/soak and installed-IDE runtime certification |
...
| Collaboration/governance | Major gap | Reviewable source output, local change history and signed exact-version project/connector organization catalogs with native sensitive-change approval | Shared model merge semantics, broader ownership/approval policies, audit export, CI policy gates and team permissions |
...
- The database/runtime certification matrix force-recreated five disposable
  engines and passed twenty Jmix/Java runtime cells plus ten production
  reverse-engineering host cells. Sixty runtime documents prove startup,
  enhancement, `DataManager`, baseline, forward/rollback/reapply and released
  locks; ten reverse-engineering documents prove project-property/library-driver
  loading, schema and constraint reconstruction, dependency closure, import
  planning and redaction on both supported IntelliJ hosts. The 70-document
  audit found zero failed assertions and no JDBC URL or password. PostgreSQL's
  clean-start missing `databasechangeloglock` log is the expected Liquibase
  existence probe; the final row was directly verified unlocked.
...
- The durable-outbox workflow was subsequently exercised through the real
  Integration Designer at 1280, 640 and 320 pixels. Kafka selection, outbox
  enablement, automatic transaction/at-least-once normalization, indexed-store
  selection and every lease/retry/retention/replay control remained available.
  The 320-pixel document and viewport widths were both exactly 320 pixels, the
  catalog and inspector ended inside the viewport, all visible form fields were
  at least 40 pixels high, and the immutable preview showed the Java adapter,
  policy and include-all Liquibase migration as three files. The explicit-root
  workspace fixture separately proves the root include is the fourth file in
  the same atomic plan. No browser warning or error was emitted.
- The persistent-inbox consumer workflow was exercised at 1280, 720 and 360
  pixels. At 360 pixels the catalog, adapter canvas and reliability inspector
  stack inside the 360-pixel document with zero horizontal overflow; the
  3,942-pixel workspace scroll reaches every inbox, retry and DLT control.
  Editing the maintenance batch and using visual Undo restored the original
  value, while the browser emitted no runtime error. Backend tests prove forged
  inbox data-source, transaction-manager and Jackson selections are overwritten
  from the indexed destination/store contract on both IntelliJ hosts.
...
- The generated-integration runtime matrix force-recreated disposable
  ...
  Micrometer delivery counters. Each cell additionally passed six Kafka and six
  RabbitMQ inbound scenarios: identical duplicate suppression, conflicting
  payload rejection for a reused ID, transient bounded retry, poison-message
  DLT, missing-ID quarantine and permission-controlled replay. The handler's
  database effect and inbox state committed transactionally.
- The same two cells now pass credential-free evidence schema v4 for an
  OAuth2 client-credentials lifecycle and a disposable private PKI. Expired
  access tokens are renewed; a resource-server `invalid_token` response evicts
  the exact authorized client so the next call obtains a new token. The
  generated mTLS client presents a trusted certificate, while missing client
  identity, untrusted server CA and hostname mismatch fail before WireMock's
  HTTP handler. Updating the named SSL bundle swaps in independently generated
  rotated key material without application restart; replacement is constructed
  before the volatile client reference changes so a failed rebuild retains the
  last working client.
- Signed organization connector catalogs now share the immutable Ed25519,
  SHA-256, exact-version, expiry, offline-cache and anti-rollback boundary used
  by project templates. A native IntelliJ workflow authors and reviews
  declarative compatibility, authentication, mTLS, reliability, telemetry,
  secret-reference sensitivity, risk and approval policy before create-only
  export. The schema rejects executable code, endpoints and secret values.
  Preview/apply reopens the exact bundle and enforces every policy field in the
  backend; sensitive/restricted connectors require a five-minute native
  capability bound to the exact digest/template/module/policy and stripped
  from generated metadata. Focused tests cover tampering, forbidden fields,
  key rotation, rollback, cache drift, compatibility, downgrade, approval
  scope/expiry and disable/offline behavior. Browser measurements at
  1200/720/360 pixels showed zero horizontal overflow and no console error.
  See `ORGANIZATION-CONNECTOR-CATALOGS.md`.
- The workflow palette, subprocess inspector, Jmix email inspector and scenario
  ...
  errors.
- IntelliJ 2025.3: 396 tests and 3 host smoke tests passed; the packaged
  plugin verifier reports compatibility with IU-253.28294.334.
- IntelliJ 2026.2: 396 tests and 3 host smoke tests passed; the packaged
...
3. Add real runtime debugging, process-instance migration and assignee/security
   context simulation.
4. Finish the integration runtime tranche: provider-native tracing/audit,
   authorization-code and RFC 8705 certificate-bound-token flows, remaining
   provider runtime fixtures, cross-database inbox/outbox proof,
   multi-node/load/soak and installed-IDE runtime proof.
